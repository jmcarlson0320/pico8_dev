...
function do_slider(id, x, y, over, min, max, val, length)
    local val = do_slider_logic(id, over, min, max, val, length)
    draw_slider(id, x, y, over, min, max, val, length)
    return val
end
...
function do_slider_logic(id, over, min, max, val, length)
    ...
    if id == active then
        ...
        if not btn(4) and not btn(5) then active = nil end
    elseif over then
...
function draw_slider(id, x, y, over, min, max, val, length)
